...
giving a short introduction into the used methodology. The package accompagnies the
papers by \citet{VanMoerbeke2017} and \citet{VanMoerbeke2018} which introduced
the REIDS model.
\section{Data}
...
\newpage
\section{Ranking of AS Exons by Junction Support}
In this section we present an example of an analysis by the REIDS model of a HTA
data set. This recently developped microarray platform contains probes designed
for exon-exon junctions. The analysis is carried out as above but is followed by
a second stage in which the identified exons are ranked according to their
junction support. A more detailed description can be found in
\cite{VanMoerbek2018}. The code for an HTA microarray is presented below.
<<HTAData,eval=FALSE>>=
library(REIDS)
...
@
\begin{verbatim}
python Line_Indexer.py --output_file HTAData_lineindex.csv HTAData_Pivot.csv
\end{verbatim} 
<<REIDSHTA,eval=FALSE>>=
load("TissueData/TissueData.RData")
...
REIDS_Output=REIDSFunction(geneData=TissueData,nsim=5000,geneID=TissueData$GeneID,
                       exonID=TissueData$ExonID,informativeCalls=TRUE,alpha=0.5)
@
\noindent As before, it is recommended to use the clusterversion of the REIDS
model, \texttt{REIDSFunction\_ClusterVersion}, for large data sets followed by
\texttt{CreateOutput}. Next, we perform a test on the array scores of the exons
and provide an annotation file of the exons and their corresponding junctions.
<<HTA_Analysis,eval=FALSE>>=
...
@
We now add the support of the junctions in order to rank the identified AS
candidates from high to low confidence.
<<HTARank,eval=FALSE>>=
